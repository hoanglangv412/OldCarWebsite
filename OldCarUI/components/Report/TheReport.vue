<template>
  <div class="TheCarversion">
    <td id="dataPosts" style="display: none">{{ dataPosts }}</td>
    <td id="dataPostCustomers" style="display: none">
      {{ dataPostCustomers }}
    </td>
    <CCard>
      <CCardHeader>
      ...
      <CCardBody class="text">
        <div class="row">
          <div class="col">
            <CCallout color="danger">
              ...
              <strong
                class="h4"
                id="soldCount"
                v-if="dataPosts.length > 0 && role == 0"
              >
              </strong>
              <strong
                class="h4"
                id="soldCountCustomers"
                v-if="dataPostCustomers.length > 0 && role == 1"
              >
              </strong>
            </CCallout>
          </div>
          ...
          <div class="col">
            <CCallout color="warning">
              ...
              <strong class="h4" v-if="dataPosts.length > 0 && role == 0"
                >{{ dataPosts.length }}
              </strong>
              <strong
                class="h4"
                v-if="dataPostCustomers.length > 0 && role == 1"
                >{{ dataPostCustomers.length }}</strong
              >
            </CCallout>
          </div>
          <div class="col">
        ...
        <CDataTable
          v-if="dataPostCustomers.length > 0 && role == 1"
          id="tablecuteCustomer"
          class=""
          :items="dataPostCustomers"
    ...
    <CCard v-if="dataPosts.length > 0 && role == 0">
      <CCardHeader>
        <CRow>
...
export default {
  ...
  updated() {
    if (!$("#dataPosts").html().includes("[]")) {
      $("#unsoldCount").text(
        $("#dataPosts")
          .html()
          .match(/"Post_car_sold": false/g) != null ? $("#dataPosts")
          .html()
          .match(/"Post_car_sold": false/g).length : 0
      );
      $("#soldCount").text(
        $("#dataPosts")
          .html()
          .match(/"Post_car_sold": true/g) != null ? $("#dataPosts")
          .html()
          .match(/"Post_car_sold": true/g).length : 0
      );
    }
    if (!$("#dataPostCustomers").html().includes("[]")) {
      $("#unsoldCountCustomers").text(
        $("#dataPostCustomers")
          .html()
          .match(/"Post_car_sold": false/g) != null ? $("#dataPostCustomers")
          .html()
          .match(/"Post_car_sold": false/g).length : 0
      );
      $("#soldCountCustomers").text(
        $("#dataPostCustomers")
          .html()
          .match(/"Post_car_sold": true/g) != null ? $("#dataPostCustomers")
          .html()
          .match(/"Post_car_sold": true/g).length : 0
      );
    }
  ...
  methods: {
    viewDetail(value) {
      if (value != undefined) {
        ...
        console.log(this.domain + "Car/GetDataById/" + value.Post_car_id);
        axios
